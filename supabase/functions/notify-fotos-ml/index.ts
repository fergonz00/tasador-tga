// Edge Function: notify-fotos-ml
// La dispara `control_fotos_ml.py`, que corre en la PC de Fer con una tarea
// programada a las 15:00. Avisa que autos con stock siguen sin poder publicarse
// en Mercado Libre porque faltan las fotos.
//
// Corre en la PC y no en Vercel porque la carpeta que controla (V:\\NADIA\\FOTOS
// AUTOS ML) es una unidad de red de la oficina, que Vercel no ve.
//
// Deja de avisar solo: cuando los 9 modelos estan completos el script no llama.
//
...
//   ML_FOTOS_DESTINATARIOS (env, default fngonzalez,nvera), activos, con
//   telefono_wa. Dedup por teléfono.
// - Template Meta: `ml_faltan_fotos` (es_AR): {{1}} primer nombre, {{2}} cuántos
//   autos faltan, {{3}} detalle en una sola línea (Meta rechaza los saltos de
//   línea dentro de un parámetro). El cuerpo va SIN TILDES.

const META_API_URL = "https://graph.facebook.com/v25.0";
const META_LANGUAGE = "es_AR";
const TEMPLATE_NAME = "ml_faltan_fotos";
...
async function procesar(env: Env, cantidad: string, detalle: string, solo: string | null) {
  ...
  const usuarios = (Deno.env.get("ML_FOTOS_DESTINATARIOS") ?? DESTINATARIOS_DEFAULT)
...
async function enviar(
  ...
  const texto = recortar(
    `📷 Mercado Libre: hay ${cantidad} autos con stock que no se pueden publicar porque faltan las fotos — ${detalle}`,
    900,
  );
...
// {{1}} primer nombre - {{2}} cuantos autos faltan - {{3}} detalle, una linea.
const TEMPLATE_COMPONENTS = [
  {
    type: "BODY",
    text:
      "Hola {{1}}! Hay {{2}} autos con stock que todavia no se pueden publicar en Mercado Libre porque faltan las fotos.\n\n{{3}}\n\nLas fotos van a la carpeta FOTOS AUTOS ML. Cuando esten completas aviso que quedaron listos.",
    example: {
      body_text: [[
        "Fer",
        "8",
        "Taos Highline: sin fotos - Nivus Comfortline: sin fotos - Tera Outfit: 3 de 6",
      ]],
    },
  },
];
